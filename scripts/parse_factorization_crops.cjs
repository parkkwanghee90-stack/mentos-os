// scripts/parse_factorization_crops.cjs
const fs = require('fs');
const path = require('path');

const BASE_MATH_CROPS = 'c:/mentos_os_clean/public/math_crops/(003)인수분해';
...
function parseStage(stageName) {
  ...
  console.log(`\n=== Processing Factorization ${stageName} ===`);
...
function main() {
  ...
  stages.forEach(stage => {
    const result = parseStage(stage);
    if (result) {
      const unitKey = `인수분해${stage}`;
      allResults[unitKey] = result;
      masterAnswers[unitKey] = result.answers;
    }
  });

  // Save the master extracted answer key for verification
  fs.writeFileSync(path.join(BASE_MATH_CROPS, 'extracted_answers.json'), JSON.stringify(masterAnswers, null, 2), 'utf8');
  console.log(`\n- Generated local extracted_answers.json inside: ${BASE_MATH_CROPS}`);
