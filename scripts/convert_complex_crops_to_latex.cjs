...
const BASE_DIR = path.join(__dirname, '../public/math_crops/(004)복소수');
const TEXT_DB_PATHS = [
...
async function processStage(stageName, keyPrefix) {
  ...
  // Process ALL webp crop files in this stage directory
  ...
  console.log(`\n--- Transcribing Complex Numbers ${stageName} (All Problems, Count: ${files.length}) ---`);
  ...
  for (const file of files) {
    ...
    while (attempts > 0 && !transcribed) {
      try {
        console.log(`  -> OCR Transcribing complex ${stageName} ${file}...`);
...
async function run() {
  const stages = [
    { name: '2단계', dbKey: '(004)복소수2단계' },
    { name: '3단계', dbKey: '(004)복소수3단계' },
    { name: '4단계', dbKey: '(004)복소수4단계' }
  ];

  for (const stage of stages) {
    await processStage(stage.name, stage.dbKey);
  }
  console.log('\n=== Complex Numbers LaTeX Transcription Completed (All Problems) ===');
}
