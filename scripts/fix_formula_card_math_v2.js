...
// Helper to create a string with literal backslashes for math
// We want the string value in JS memory to have ONE backslash before commands.
// e.g. "\alpha"
// To get that in a JS literal, we use "\\alpha".
// BUT for "\times", we must use "\\\\times" because "\t" is tab.
// Actually, even better: use a replace to fix the "tab" etc.
const fixMath = (str) => {
  return str
...
// I will just hardcode the strings correctly.
// REMEMBER: In a JS literal, "\\alpha" -> "\alpha".
// BUT "\\times" -> "\times" is ONLY if it's not interpreted as tab.
// In JS, "\t" IS tab. So I MUST use "\\\\times" if I want the string value to be "\times".
...
// Wait! I missed some backslashes in the trig part above.
// For \times, I used \\\\times. Let's be careful.
// Actually, I'll rewrite it using String.raw to be SAFE.

trig.content = String.raw`# 📌 삼각함수의 성질 완벽 정리
...
exp.content = String.raw`# 📌 지수함수 완벽 정리
...
log.content = String.raw`# 📌 로그함수 완벽 정리
...
console.log('Fixed Formula Card with String.raw (single backslash in memory)!');
