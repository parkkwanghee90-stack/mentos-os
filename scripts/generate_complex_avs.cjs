...
const MANIFEST_PATH = path.join(__dirname, '../public/math_crops/(004)복소수/extracted_answers.json');
...
- B: 필요한 배경개념/공식 (Relevant concepts/formulas like definition of imaginary unit i, complex conjugates, properties of complex numbers arithmetic, equality of complex numbers)
- S: 식 변형/구조 분석 (Algebraic transformation, Grouping real and imaginary parts, Simplification)
- A: 계산 적용 및 최종 정답 도출 (Final calculation and complex number deduction)
...
async function processStage(stageName, dbKey, textDbPrefix) {
  if (!fs.existsSync(MANIFEST_PATH)) {
    console.error(`[ERROR] Answers manifest missing at: ${MANIFEST_PATH}`);
    return;
  }
  
  const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf8'));
  const stageData = manifest['복소수' + stageName];
  if (!stageData) {
    console.warn(`[WARNING] No manifest data found for: 복소수${stageName}`);
    return;
  }

  // Process all available questions in this stage
  ...
  console.log(`\n--- Generating AVS for 복소수 ${stageName} (All Problems, Count: ${qCount} problems) ---`);
  ...
  for (let i = 1; i <= qCount; i++) {
    ...
    while (attempts > 0 && !avsObj) {
      try {
        console.log(`  -> Generating PCBSA JSON for 복소수 ${stageName} ${id}...`);
  ...
  console.log(`Finished AVS for 복소수 ${stageName}: Generated: ${successCount}, Skipped: ${skipCount}, Missing Problem Text: ${missingTextCount}`);
}

async function run() {
  const stages = [
    { name: '2단계', dbKey: '복소수2단계', textDbPrefix: '(004)복소수2단계' },
    { name: '3단계', dbKey: '복소수3단계', textDbPrefix: '(004)복소수3단계' },
    { name: '4단계', dbKey: '복소수4단계', textDbPrefix: '(004)복소수4단계' }
  ];

  for (const stage of stages) {
    await processStage(stage.name, stage.dbKey, stage.textDbPrefix);
  }
  console.log('\n=== Complex Numbers AVS PCBSA Hint JSON Generation Completed (All Problems) ===');
}
