...
async function uploadFile(localAbsPath, remotePath) {
  ...
  if (res.ok) {
    ...
    console.log(`❌ Failed to upload ${remotePath}`);
  }
}

async function main() {
  await uploadFile('C:\\mentos_os_clean\\assets_backup\\data\\answers_master.json', 'data/answers_master.json');
  await uploadFile('C:\\mentos_os_clean\\assets_backup\\data\\avs_answers.json', 'data/avs_answers.json');
}
