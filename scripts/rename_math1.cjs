    ...
    for (const unit of units) {
        ...
        for (const num of Object.keys(groups)) {
            ...
            if (sorted.length >= 2) {
                ...
                if (oldQ !== newQ && fs.existsSync(oldQ)) { fs.renameSync(oldQ, newQ); qRenamed++; }
                if (oldA !== newA && oldA !== oldQ && fs.existsSync(oldA)) { fs.renameSync(oldA, newA); aRenamed++; }
