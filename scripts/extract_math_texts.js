...
async function main() {
  const targetFolders = [
    { base: '(5)수학1 중간/2단계', name: '삼각함수활용2단계', alias: '삼각함수활용2단계' }
  ];
  ...
  for (const target of targetFolders) {
    ...
    const filesToProcess = files; // Process all files in the folder
    
    const tasks = filesToProcess.map(file => limit(async () => {
