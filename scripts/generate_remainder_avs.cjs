...
const MANIFEST_PATH = path.join(__dirname, '../public/math_crops/(002)항등식과나머지정리/extracted_answers.json');
...
- B: 필요한 배경개념/공식 (Relevant concepts/formulas like Algebraic Identities, Remainder Theorem, Factor Theorem, Synthetic Division)
- S: 식 변형/구조 분석 (Algebraic transformation or Remainder Theorem setup)
- A: 계산 적용 및 최종 정답 도출 (Final calculation and answer deduction)
...
async function processStage(stageName, dbKey, textDbPrefix) {
  ...
  const stageData = manifest[stageName];
  if (!stageData) return;
  ...
  console.log(`\n--- Generating AVS for ${stageName} (${stageData.qCount} problems) ---`);
  ...
  for (let i = 1; i <= stageData.qCount; i++) {
    ...
    const correctAnswer = stageData.answers[i];
    if (!correctAnswer) {
      console.warn(`[WARNING] No answer found for ${id} in ${stageName}, skipping.`);
      continue;
    }

    const isMultipleChoice = ['1', '2', '3', '4', '5'].includes(correctAnswer);
    ...
    if (avsObj) {
      ...
      successCount++;
      console.log(`[SUCCESS] Generated AVS for ${id} (${successCount}/${stageData.qCount})`);
      await sleep(1000);
    } else {
...
async function run() {
  const stages = [
    { name: '2단계', dbKey: '항등식과나머지정리2단계', textDbPrefix: '(002)항등식과나머지정리2단계' },
    { name: '3단계', dbKey: '항등식과나머지정리3단계', textDbPrefix: '(002)항등식과나머지정리3단계' },
    { name: '4단계', dbKey: '항등식과나머지정리4단계', textDbPrefix: '(002)항등식과나머지정리4단계' }
  ];
