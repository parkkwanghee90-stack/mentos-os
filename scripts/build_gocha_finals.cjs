...
const CROP_DIR = 'c:\\mentos_os_clean\\public\\math_crops\\매쓰플랫_ultimate';
const OUT_DIR = 'c:\\mentos_os_clean\\public\\math_indexed\\고차방정식\\concept';
const PDF_BASE_DIR = '\\\\Subitmainpc\\수학의 빛 사무폴더\\@@매쓰플랫\\(2)수학(상)기말';

if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

async function run() {
    const folders = fs.readdirSync(CROP_DIR);
    let matchedCount = 3; // start from current 3 questions
    
    for (const folder of folders) {
        if (!folder.includes('기말') || !folder.includes('쌍둥이')) continue; 
        ...
        console.log(`Found ${matches.length} high-order problems: `, matches);
            ...
            for (const m of matches) {
                ...
                if (pImg) {
                    ...
                    if (fs.existsSync(srcPath)) {
                        fs.copyFileSync(srcPath, destQ);
                        fs.copyFileSync(srcPath, destS);
    ...
    console.log(`\nSuccess! Total ${matchedCount - 3} MORE high-order problems extracted.`);
}
