...
const MANIFEST_PATH = path.join(__dirname, '../public/math_crops/(001)다항식/extracted_answers.json');
...
- B: 필요한 배경개념/공식 (Relevant concepts/formulas)
- S: 식 변형/구조 분석 (Algebraic transformation)
...
async function processStage(stageName, dbKey, textDbPrefix) {
  ...
  // Reload text DB to get newly transcribed problems
  ...
  for (let i = 1; i <= stageData.qCount; i++) {
    ...
    if (avsObj) {
      // Complete some fields on our side just to be safe
...
async function run() {
  const stages = [
    { name: '2단계', dbKey: '다항식2단계', textDbPrefix: '(001)다항식2단계' },
    { name: '3단계', dbKey: '다항식3단계', textDbPrefix: '(001)다항식3단계' },
    { name: '4단계', dbKey: '다항식4단계', textDbPrefix: '(001)다항식4단계' }
  ];
...
// Export for scheduled run or run directly
