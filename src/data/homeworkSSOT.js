...
/**
 * 숙제에서 제외할 문항 (풀이 이미지·정답이 모두 없어 채점 불가한 문제).
 * 크롭 파이프라인 off-by-one으로 풀이 크롭이 누락되어 정답 생성/AI풀이 모두 실패한 14문항.
 * + 2026-06-11 수1·수2 전수 감사: 섹션(2단계/3·4단계) 경계에서 직전 문제 해설이 새 슬롯으로
 *   복제된 단원 중간 유령 슬롯 21문항 추가(문제 이미지=직전 해설 복제, 자체 해설 없음,
 *   저장 정답=직전 정답 복제 — 비전 직판독 전건 확정, scripts/homework_avs_asset_issues.json).
 * key = answerKey, value = 제외할 문제번호 배열.
 */
export const EXCLUDED_PROBLEMS = {
  '수학1_01지수_통합숙제': [36, 46],
  '수학1_02로그_통합숙제': [24, 43],
  '수학1_03지수로그함수_통합숙제': [36, 72],
  '수학1_04지수로그함수활용_통합숙제': [17, 32],
  '수학1_05삼각함수정의_통합숙제': [19, 33],
  '수학1_06삼각함수그래프_통합숙제': [38, 74],
  '수학1_07삼각함수활용_통합숙제': [16, 44],
  '수학1_08등차등비수열_통합숙제': [18, 31],
  '수학1_09수열의합_통합숙제': [42, 69],
  '수학1_10수학적귀납법_통합숙제': [25, 45],
  '수학2_01함수의극한_통합숙제': [20, 46],
  '수학2_02함수의연속_통합숙제': [16, 34],
  '수학2_03미분계수_통합숙제': [16, 37],
  '수학2_04도함수활용12_통합숙제': [9, 21, 43],
  '수학2_05도함수활용3_통합숙제': [12, 42],
  '수학2_06부정적분정적분_통합숙제': [24, 69],
  '수학2_07정적분활용_통합숙제': [13, 21],
};

/** 해당 문항이 제외 대상인지 */
export function isExcludedProblem(answerKey, num) {
  const list = EXCLUDED_PROBLEMS[answerKey];
  return Array.isArray(list) && list.includes(Number(num));
}
