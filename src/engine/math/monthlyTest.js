...
export function sendMonthlyParentPush(studentName, gradingResult) {
  ...
  const _byAcc = [...gradingResult.unitDiagnoses].sort((a, b) => b.testAccuracy - a.testAccuracy);
  const _gradeOf = (s) => (s >= 90 ? '1등급' : s >= 80 ? '2등급' : s >= 70 ? '3등급' : s >= 60 ? '4등급' : '5등급 이하');
  queueParentPush(msg, {
    templateKey: 'monthlyTest', // 월말평가리포트
    variables: {
      '#{name}': studentName,
      '#{avg}': String(gradingResult.accuracy),
      '#{grade}': _gradeOf(gradingResult.accuracy),
      '#{strong}': _byAcc[0] ? _byAcc[0].unit : '-',
      '#{weak}': _byAcc.length ? _byAcc[_byAcc.length - 1].unit : '-',
      '#{goal}': gradingResult.accuracy >= 80 ? '상위권 안정화 및 심화 학습' : '취약 단원 집중 보강',
    },
  });
  return msg;
}
