...
export default function PremiumLecturePlayer({ lectureId, onClose }) {
  ...
  return (
    <div className="white-bg-katex" style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: '#ffffff', color: '#000000', zIndex: 99999, overflow: 'hidden', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', background: '#ffffff' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: '900', color: '#000000', margin: 0 }}>Premium AI Lecture</h2>
          <p style={{ fontSize: '0.875rem', fontWeight: 'bold', color: '#2563eb', margin: '4px 0 0 0' }}>STEP {currentStep + 1} / {lectureData.steps.length}</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <button 
            ...
            style={{ background: 'linear-gradient(to right, #8b5cf6, #3b82f6)', border: 'none', padding: '0.6rem 1.5rem', borderRadius: '12px', color: 'white', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: '900', fontSize: '1rem', letterSpacing: '0.5px', whiteSpace: 'nowrap', boxShadow: '0 4px 15px rgba(139, 92, 246, 0.4)' }}
          >
            ✨ AI Vision
          </button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem', background: '#ffffff', overflowY: 'auto' }}>
        <AnimatePresence mode="wait">
          ...
          >
            <h3 style={{ fontSize: '2rem', fontWeight: '900', color: '#000000', marginBottom: '2rem', textAlign: 'center' }}>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem' }}>
              ...
              {stepData.visuals.component && (
                <div style={{ width: '100%', display: 'flex', justifyContent: 'center', marginBottom: '2rem', minHeight: '350px' }}>
              ...
              {stepData.visuals.question && (
                <div style={{ width: '100%', maxWidth: '900px', marginBottom: '1rem' }}>
                  <div style={{ background: '#2563eb', color: '#ffffff', padding: '8px 24px', borderRadius: '12px 12px 0 0', fontSize: '0.85rem', fontWeight: '900', letterSpacing: '0.1em' }}>QUESTION</div>
                  <div style={{ background: '#eff6ff', width: '100%', padding: '2rem', borderRadius: '0 0 12px 12px', border: '2px solid #2563eb', color: '#1e40af', overflowX: 'auto' }}>
              ...
              {stepData.visuals.solution && (
                <div style={{ width: '100%', maxWidth: '900px', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10 }}>
                   <div style={{ background: '#1e293b', color: '#ffffff', padding: '8px 24px', borderRadius: '12px 12px 0 0', fontSize: '0.85rem', fontWeight: '900', letterSpacing: '0.1em' }}>SOLUTION STEPS</div>
                   <div style={{ background: '#ffffff', width: '100%', padding: '2rem', borderRadius: '0 0 2rem 2rem', border: '3px solid #1e293b', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', overflowX: 'auto' }}>
                      <div style={{ fontSize: '1.35rem', color: '#000000' }}>
              ...
              {stepData.visuals.math && !stepData.visuals.question && (
                <div style={{ width: '100%', maxWidth: '900px', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10 }}>
                   <div style={{ background: '#2563eb', color: '#ffffff', padding: '8px 24px', borderRadius: '12px 12px 0 0', fontSize: '0.85rem', fontWeight: '900', letterSpacing: '0.1em' }}>Premium Problem & Solution</div>
                   <div style={{ background: '#ffffff', width: '100%', padding: '2rem', borderRadius: '0 0 2rem 2rem', border: '3px solid #2563eb', boxShadow: '0 10px 30px rgba(37, 99, 235, 0.1)', overflowX: 'auto', color: '#000000' }}>
                      <div style={{ fontSize: '1.25rem', color: '#000000', textAlign: 'left' }}>
      ...
      <div style={{ padding: '2.5rem', background: '#ffffff', borderTop: '1px solid #e2e8f0' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ width: '100%', background: '#f8fafc', padding: '2rem', borderRadius: '1.5rem', marginBottom: '2rem', border: '1px solid #f1f5f9' }}>
            <p style={{ fontSize: '1.75rem', fontWeight: '700', lineHeight: '1.6', color: '#000000', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', margin: 0 }}>
              <Volume2 style={{ color: '#2563eb' }} size={32} />
              <span>{renderNarration(stepData.narration)}</span>
            </p>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '3rem' }}>
            <button onClick={prevStep} disabled={currentStep === 0} style={{ background: 'none', border: 'none', color: currentStep === 0 ? '#e2e8f0' : '#94a3b8', cursor: 'pointer' }}><ChevronLeft size={50} /></button>
            <button onClick={handlePlayPause} style={{ width: '6rem', height: '6rem', background: '#2563eb', color: '#ffffff', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 25px rgba(37, 99, 235, 0.3)' }}>
              {isPlaying ? <Pause size={40} fill="currentColor" /> : <Play size={40} className="ml-1" fill="currentColor" />}
            </button>
            <button onClick={nextStep} disabled={currentStep === (lectureData?.steps?.length || 0) - 1} style={{ background: 'none', border: 'none', color: currentStep === (lectureData?.steps?.length || 0) - 1 ? '#e2e8f0' : '#94a3b8', cursor: 'pointer' }}><ChevronRight size={50} /></button>
          </div>
        </div>
      </div>
