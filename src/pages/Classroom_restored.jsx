...
export default function Classroom() {
  ...
  const pendingResetRef = useRef(location.state?.forceNew === true || (localStorage.getItem('mentos_last_teacher_id') && localStorage.getItem('mentos_last_teacher_id') !== (location.state?.teacher?.id || 'unknown')));
  ...
  useEffect(() => {
    const sessionPrevTeacherId = localStorage.getItem('mentos_last_teacher_id');
    const nextTeacherId = currentTeacher?.id || 'unknown';
    const isForceNew = location.state?.forceNew === true;
    
    if (isForceNew || (sessionPrevTeacherId && sessionPrevTeacherId !== nextTeacherId)) {
       console.log(`[CLASS RESET]
prevTeacher=${sessionPrevTeacherId}
nextTeacher=${nextTeacherId}
...
cacheCleared=true`);
       ...
       // Once consumed, remove the flag from location state to prevent loop on hot-reloads
       if (isForceNew) {
  ...
  }, [currentTeacher?.id, location.state?.forceNew]);
...
lessonSet=[Dynamic Generation based on contentRules: ${currentTeacher.contentRules?.[0]?.substring(0,10) || 'default'}...]');
  ...
  const createLessonStart = ({ subject, mode, teacher, teacherMode, contentProfile, unit }) => {
    ...
    if (teacher?.name === "백도진") {
      console.log("[LESSON HARD LOCK]
teacher=백도진_mode=teacher_only
genericBlocked=true");
      return `[수업 시작 강제 지시서 - 백도진 선생님]
반드시 아래 내용으로만 수업을 시작하라. 다른 템플릿 문법 설명 등 절대 예시 금지.

학생: 안녕하세요, 선생님!
선생님: 안녕! 백도진 쌤이야! 너 ${unit || '기초 문법'} 배우러 왔구나? 오늘은 딱 한 문장만 진짜 재밌게 씹어먹어 볼 테니까 기대해! 자, 화면에 나오는 짧은 이야기 먼저 편하게 읽어보자!
...
이 대사로만 시작할 것.`;
    ...
    if (teacher?.name === "설윤아") {
      console.log("[LESSON HARD LOCK]
teacher=설윤아_mode=teacher_only
genericBlocked=true`);
      return `[수업 시작 강제 지시서 - 설윤아 선생님]
반드시 아래 내용으로만 수업을 시작하라. 기초 문법 설명 금지. 초등 회화 금지. 고1/고2 1~2등급 최상위권 대상 실전 독해(수능형) 지도로 시작하라.

학생: 안녕하세요, 선생님!
선생님: 안녕! 설윤아 쌤이야! 너 ${unit && unit !== 'undefined' ? unit : '상위권 독해 실전'} 강의 들으러 왔구나? 오늘은 고난도 지문의 핵심 정보 구조를 정확하게 파고들 거야. 초집중해서 따라와! 자, 화면에 나오는 지문부터 분석해 볼까?

이 대사로만 시작할 것.`;
    }

    const subjectPrefix = (subject === 'english' || subject === '영어') ? '실전 독해 지문' : (unit || '기본');
    const routeDesc = contentProfile ? `[${contentProfile} 기반 수업]` : `[${subjectPrefix} 수업 시작]`;
    
    return `${routeDesc}
선생님: 안녕! 나는 ${teacher?.name || '선생님`}이야! 오늘은 ${subjectPrefix} 파트를 공부할 거야. ${teacherMode ? `(${teacherMode} 방식으로 진행할게)` : ''} 화면의 내용을 읽어보고 모르는 부분이 있으면 질문해줘!';
...
mode=${mode}`);
  ...
  const { session, remaining, recommendedPhase, resumedFromCheckpoint, patchSession, finishSession } = useSession({
    ...
    studyMode: mode,
    forceNew: location.state?.forceNew === true || pendingResetRef.current,
    studentState,
    messages,
...
generated_questions=5`);
  ...
  useEffect(() => {
    ...
    if (!isLessonStarted && !loading) {
      if (!showUploadPrompt) {
        const normalizedSubject = String(currentSubject || '')
  .trim()
  .toLowerCase();

const englishOrScienceSubjects = new Set([
  'eng',
  'english',
  'physics',
  'chemistry',
...
const isEnglishOrScience = englishOrScienceSubjects.has(normalizedSubject);
        if (isEnglishOrScience && !location.state?.isResume && session.messages?.length === 0) {
           setShowUploadPrompt(true);
           if (ttsEnabled) {
             speakText("?ㅻ뒛 ?숆탳?먯꽌 諛쏆? ?꾨┛?몃Ъ???덉쑝硫??ш린 ?낅줈?쒗빐 二쇱꽭?? ???꾨┛??湲곕컲?쇰줈 ?쒗뿕 ?鍮꾨? ?꾩??쒕┫寃뚯슂.", { voice: getVoiceForTeacher(currentTeacher), subject: currentSubject });
           }
           return;
        }
      } else {
      ...
      setIsLessonStarted(true); // 以묐났 ?ㅽ뻾 諛⑹?
