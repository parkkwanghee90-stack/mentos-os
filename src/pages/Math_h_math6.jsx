...
import PcbsChatWidget from '@/components/PcbsChatWidget';
...
import { mathTextsData, loadMathData } from '@/services/mathDataLoader';
...
function LessonRenderer({ session, setSession, ssot, timeLeft, selectedUnit, testProblemIdx, setTestProblemIdx, messages, setMessages }) {
  ...
  // PCBS 위젯 상태
  const [showPcbsWidget, setShowPcbsWidget] = useState(false);
  ...
  const getHintFolder = (unitName) => {
    if (!unitName) return null;
    const clean = unitName.replace(/\s+/g, '');
    const stepMatch = clean.match(/(2|3|4)단계/);
    const stepStr = stepMatch ? stepMatch[0] : '2단계';
    
    // 고1 및 공통 매핑 (MathClassroom과 일치시킴)
    if (clean.includes('다항식')) return `poly_s${stepStr[0]}`;
    if (clean.includes('항등식') || clean.includes('나머지정리')) return `remain_s${stepStr[0]}`;
    if (clean.includes('인수분해')) return `factor_s${stepStr[0]}`;
    if (clean.includes('복소수')) return `complex_s${stepStr[0]}`;
    if (clean.includes('이차방정식과이차함수') || clean.includes('이차방정식과이차함수')) return `quad_func_s${stepStr[0]}`;
    if (clean.includes('이차방정식')) return `quad_eq_s${stepStr[0]}`;
    
    if (clean.includes(`행렬`)) return `matrix_s${stepStr[0]}`;
    if (clean.includes(`고차방정식`)) return `higher_eq_s${stepStr[0]}`;
    if (clean.includes(`일차부등식`)) return `linear_ineq_s${stepStr[0]}`;
    if (clean.includes(`이차부등식`)) return `quad_ineq_s${stepStr[0]}`;
    if (clean.includes(`경우의수`)) return `cases_s${stepStr[0]}`;
    if (clean.includes(`점과좌표`)) return `point_s${stepStr[0]}`;
    if (clean.includes(`직선의방정식`)) return `line_s${stepStr[0]}`;
    if (clean.includes('원의방정식')) return `circle_s${stepStr[0]}`;
    if (clean.includes('도형의이동')) return `shape_move_s${stepStr[0]}`;
    ...
    // 미적분 (Calculus) 지원 추가
    if (clean.includes('수열의극한')) return stepStr === '4단계' ? '1)극한4단계' : `1)극한${stepStr.replace('단계','')}`;
    if (clean.includes('급수')) return stepStr === '4단계' ? '2)급수4단계' : `2)급수${stepStr.replace('단계','')}`;
    if (clean.includes('지수로그함수의극한') || clean.includes('지수로그함수극한')) return '3)지수로그함수의극한';
    
    if (clean.includes('지수함수')) return stepStr === '4단계' ? '지수로그함수4단계' : `지수함수${stepStr}`;
    if (clean.includes('로그함수')) return stepStr === '4단계' ? '지수로그함수4단계' : `로그함수${stepStr}`;
    if (clean.includes('지수')) return stepStr === '4단계' ? '지수로그4단계' : `지수${stepStr}`;
    if (clean.includes('로그')) return stepStr === '4단계' ? '지수로그4단계' : `로그${stepStr}`;
    return unitName;
  }
  ...
  if (session.curriculumData?.lessonContent) {
    ...
    if (phaseData) {
      ...
      // [긴급 대응] 실제 수학 DB(math_crops) 경로 매핑
  ...
  if (selectedUnit) {
     ...
     // 미적분 (Calculus) 지원 추가
     else if (currentUnit.includes('수열의극한') || currentUnit.includes('수열의 극한')) currentProblemImage = window.resolveAsset(`/math_crops/미적분/${stepStr}/calc1/${formattedIdx}.webp`)
     else if (currentUnit.includes('급수')) currentProblemImage = window.resolveAsset(`/math_crops/미적분/${stepStr}/calc2/${formattedIdx}.webp`)
     else if (currentUnit.includes('지수로그함수의극한') || currentUnit.includes('지수로그함수극한') || currentUnit.includes('지수로그함수의 극한')) currentProblemImage = window.resolveAsset(`/math_crops/미적분/${stepStr}/calc3/${formattedIdx}.webp`)
     ...
     else if (currentUnit.includes('경우의 수') || currentUnit.includes('경우의수')) currentProblemImage = window.resolveAsset(`/math_crops/(2)수학(상)기말/경우의수${stepStr}/${formattedIdx}.webp`)
     else if (currentUnit.includes(`행렬`)) currentProblemImage = window.resolveAsset(`/math_crops/(2)수학(상)기말/행렬${stepStr}/${formattedIdx}.webp`)
  ...
  if (currentProblemImage) {
      ...
      // [FALLBACK] resolveAsset이 한글→영어 변환하여 키가 안 맞는 경우, 원본 단원명(currentUnit)으로 직접 lookup
      if (!currentProblemText && mathTextsData && currentUnit) {
          ...
          if (!currentProblemText) {
              const koToEng = {
                '고차방정식2단계':'고차방정식2단계','고차방정식3단계':'고차방정식3단계','고차방정식4단계':'고차방정식4단계',
                '일차부등식2단계':'일차부등식2단계','일차부등식3단계':'일차부등식3단계','일차부등식4단계':'일차부등식4단계',
                '이차부등식2단계':'이차부등식2단계','이차부등식3단계':'이차부등식3단계','이차부등식4단계':'이차부등식4단계',
                '경우의수2단계':'경우의수2단계','경우의수3단계':'경우의수3단계','경우의수4단계':'경우의수4단계',
                '행렬2단계':'행렬2단계','행렬3단계':'행렬3단계','행렬4단계':'행렬4단계',
                '점과좌표2단계':'점과좌표2단계','점과좌표3단계':'점과좌표3단계','점과좌표4단계':'점과좌표4단계',
                '직선의방정식2단계':'직선의방정식2단계','직선의방정식3단계':'직선의방정식3단계','직선의방정식4단계':'직선의방정식4단계',
                '원의방정식2단계':'원의방정식2단계','원의방정식3단계':'원의방정식3단계','원의방정식4단계':'원의방정식4단계',
                '도형의이동2단계':'도형의이동2단계','도형의이동3단계':'도형의이동3단계','도형의이동4단계':'도형의이동4단계',
                ...
                '삼각함수활용2단계':'trig_util_step2','삼각함수활용3단계':'trig_util_step3','삼각함수활용4단계':'trig_util_step4','삼각함수활용 4단계':'trig_util_step4','삼각함수활용 4단계(68)':'trig_util_step4',
              ...
              const cleanUnit = currentUnit.replace(/\s+/g, '');
              const matchedKey = Object.keys(koToEng).find(k => k.replace(/\s+/g, '') === cleanUnit);
              const engUnit = matchedKey ? koToEng[matchedKey] : null;
              if (engUnit) {
      ...
      // [FALLBACK] 최종적으로 모든 키들을 탐색
      if (!currentProblemText && mathTextsData && currentUnit) {
          const cleanCurrent = currentUnit.replace(/\s+/g, '').replace(/[(]\d+[)]/g, '').replace(/개념/g, '');
  ...
  useEffect(() => {
    ...
    fetch(`/math_hints/${encodedFolder}/${pid}.json?v=cb_${Date.now()}`)
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(window.resolveAsset('/concept_cards/animations_math6.json')).then(res => res.json()).catch(() => ({}))
  ...
  // ── [슈퍼 관리자 검수용] 강제 스킵 ──
  const testAdvance = () => {
    // 페이즈를 넘어가는 것이 아니라 현재 페이즈 안에서 문제 번호만 +1 증가시킵니다.
  ...
  const handleSubmit = async () => {
    ...
    // 오토롤링(미리 구축된 PCBS 가이드) 동작 중일 때는 학생 질문 위주의 대화이므로, 
    // 기존처럼 '턴 횟수' 기반으로 P->C->B->S 억지 전진하는 것을 막습니다.
        ...
        if (evalStr.includes('CORRECT')) {
           ...
           if (problemStatus === 'solving') {
             ...
             setMessages(prev => {
                ...
                if (currentUnit && (currentUnit.includes('지수') || currentUnit.includes('로그'))) {
                   return [...cleaned, { 
                     role: 'assistant', 
                     content: '앗, 아쉽게도 오답입니다! 😅\n\n구체적으로 어떤 식을 세웠는지 저에게 질문 형태로 알려주시면 선생님이 교정해줄게요!'
                   }];
                }
           ...
           // QUESTION 인 경우 - 아래의 풀 API 호출로 넘김
      ...
      setTimeout(() => {
        ...
        setMessages(prev => {
          ...
          if (currentUnit && (currentUnit.includes('지수') || currentUnit.includes('로그'))) {
             return [...cleaned, { 
               role: 'assistant', 
               content: '어느 부분이 막히셨나요? 구체적으로 알려주시면 선생님이 단계별로 힌트를 드릴게요!'
             }];
          }
      ...
      <div style={{ flex: 1, padding: '2rem', overflowY: 'auto' }}>
        {chalkboardData ? (
          ...
          <div style={{ padding: '1.5rem', background: 'white', borderRadius: '12px', marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
            <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, color: '#1e3a8a', fontWeight: 'bold' }}>
                [{currentProblemTitle || '오늘의 실전 문제'}]
              </h3>
              {currentProblemText && currentProblemImage && (
                <button 
                  onClick={() => setShowOriginal(!showOriginal)}
                  style={{ padding: '0.4rem 0.8rem', background: showOriginal ? '#ef4444' : '#3b82f6', color: 'white', border: 'none', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
                >
                  {showOriginal ? '원본 닫기' : '원본보기'}
                </button>
              )}
            </div>
            ...
            {currentProblemImage && (showOriginal || !currentProblemText) && (
              <div style={{ width: '100%', display: 'flex', justifyContent: 'center', marginTop: '1rem', borderTop: showOriginal && currentProblemText ? '1px dashed #cbd5e1' : 'none', paddingTop: showOriginal && currentProblemText ? '1rem' : 0 }}>
                <img 
...
export default function MathClassroomScreen() {
  ...
  const rawTeacher = location.state?.teacher || { id: 'h_math6', name: '윤수아 선생님', courseName: '수학2 (미적분 기초)', targetGrades: ['고2'], targetRanks: ['3등급'] };
  ...
  const [selectedCourse, setSelectedCourse] = useState('수2'); // 수1/수2 선택 상태 추가
  const toggleSection = (sec) => setOpenSections(prev => ({ ...prev, [sec]: !prev[sec] }));

  // 강좌 정보 (고2 선생님 - 수학1/대수 전용)
    ...
    if (selectedCourse === '수2') {
        return {
          ...
          sections: [
            ...
            { name: `정적분의 활용`, items: levels.filter(l => l !== '2단계').map(l => `정적분의 활용${l}`) }
          ]
        };
    } else if (selectedCourse === '모의고사') {
        return {
          title: '고3 모의고사',
          sections: [
            { name: '수능', items: ['2023 수능', '2024 수능', '2025 수능'] },
            { name: '월별 모의고사', items: ['3월 모의고사', '6월 모의고사', '9월 모의고사'] },
            { name: '자체 모의고사', items: ['자체 모의고사 1회', '자체 모의고사 2회', '자체 모의고사 3회'] }
          ]
        };
    } else {
        return {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#09090b', color: 'white', position: 'relative' }}>
                      ...
                      <div 
                        ...
                        onClick={() => {
                          if (selectedCourse === '모의고사') {
                             // Navigate to Mock Exam page using title matching
                             navigate('/class/mock-exam', { state: { title: sub, elective: 'calculus' } });
                             return;
                          }
                          setSelectedUnit(sub);
      ...
      <LessonRenderer session={session} setSession={setSession} ssot={teacher} timeLeft={timeLeft} selectedUnit={selectedUnit} testProblemIdx={testProblemIdx} setTestProblemIdx={setTestProblemIdx} messages={messages} setMessages={setMessages} />
    </div>
  );
}
